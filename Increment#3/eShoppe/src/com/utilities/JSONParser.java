	...
	public ArrayList<JSONObject> getStoresLocation(String placesJson)
	{
		...
		try {
			...
			if(status.equals("OK")){
				...
				{
					storeLocations.add(results.getJSONObject(i).getJSONObject("geometry").getJSONObject("location"));
					storeLocations.get(i).put("name", results.getJSONObject(i).getString("name"));
					storeLocations.get(i).put("formatted_address", results.getJSONObject(i).getString("formatted_address"));
				}
				System.out.println("Number of items in store Locations array: " + storeLocations.size());
